...
import type { SocialPostInput } from "@sfvypaa/content";
  ...
  return (
    <form action={formAction} className="grid gap-5">
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <DateField defaultValue={post.postDate} error={fieldErrors.postDate} />
        <Field
          label="Instagram URL"
          name="instagramUrl"
...
function DateField({
  ...
  const errorId = error ? "social-post-date-error" : undefined;

  return (
    <div className="relative grid gap-2">
      <span
        className="text-sm font-semibold text-white/72"
        id="social-post-date-label"
      >
        Post date
      </span>
      <input
      ...
      <Button
        ...
        aria-labelledby="social-post-date-label"
        className={cn(
...
function ImageUploadField({
  ...
  const errorId = displayError ? "social-post-image-error" : undefined;
  ...
  return (
    <label className="grid gap-2">
      ...
      <input
        ...
        readOnly
      />
      <div className="grid gap-3 rounded-[8px] border border-white/15 bg-white p-3 text-[#171310] sm:grid-cols-[10rem_1fr]">
        <div className="relative flex aspect-square items-center justify-center overflow-hidden rounded-[8px] bg-[#f5eee5]">
          {previewUrl ? (
            <Image
        ...
        <div className="grid content-center gap-2">
          ...
          <p className="text-xs leading-5 text-[#6f655b]">
            PNG, JPG, GIF, or WebP. Maximum 6 MB. Use a square screenshot for
            best results.
          </p>
          {previewUrl ? (
            <Button
    ...
    <div className="sticky bottom-0 z-10 -mx-5 border-t border-white/10 bg-[#171310]/95 p-4 backdrop-blur sm:-mx-7 sm:px-7">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <p className="text-sm leading-6 text-white/58">
          {isPublished
            ? "Publishing updates the public homepage social feed."
