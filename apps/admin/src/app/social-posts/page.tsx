    ...
    <AdminShell active="social-posts">
      <section className="grid gap-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h1 className="text-4xl font-black tracking-normal">Social Posts</h1>
          ...
          <label className="grid gap-2">
            ...
            <div className="relative">
              ...
              <Input
                ...
                defaultValue={query}
                name="q"
                placeholder="Title, caption, Instagram URL"
              />
            </div>
          </label>
          <label className="grid gap-2">
        ...
        <div className="overflow-hidden rounded-[8px] border border-white/10 bg-white text-[#171310]">
          <div className="flex items-center justify-between border-b border-[#171310]/10 px-4 py-3">
            <p className="text-sm font-semibold text-[#5e554c]">
              {filteredPosts.length} of {posts.length} social posts
            </p>
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[62rem] text-left text-sm">
              <thead className="bg-[#f5eee5] text-xs uppercase tracking-normal text-[#6f655b]">
                <tr>
                  <th className="px-4 py-3 font-black">Post</th>
                  <th className="px-4 py-3 font-black">Status</th>
                  <th className="px-4 py-3 font-black">Post date</th>
                  <th className="px-4 py-3 font-black">Image</th>
              ...
              <tbody className="divide-y divide-[#171310]/10">
                {filteredPosts.length > 0 ? (
                  filteredPosts.map((post) => (
                    <tr className="align-top transition hover:bg-[#fbfaf8]" key={post.id}>
                      ...
                      <td className="px-4 py-4 text-[#5e554c]">
                        {post.imageUrl ? "Uploaded" : "Missing"}
                      </td>
                      <td className="px-4 py-4 text-[#5e554c]">
                      ...
                      <td className="px-4 py-4">
                        <div className="flex justify-end gap-2">
                          <Button
                            ...
                            nativeButton={false}
                            render={<Link href={`/social-posts/${post.id}`} />}
                            variant="outline"
                          >
                  ...
                  <tr>
                    <td className="px-4 py-10 text-center text-[#6f655b]" colSpan={6}>
                      No social posts match these filters.
                    </td>
                  </tr>
                )}
              </tbody>
